...
RegisterNetEvent('atmrobbery:client:roubar', function()
    RobAtm()
end)
...
function RobAtm()
	...
	if LocalPlayer.state.isLoggedIn then
		QBCore.Functions.TriggerCallback("atmrobbery:Cooldown", function(cooldown)
				...
				if CurrentCops >= 0 then
					QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
						if hasItem then
							...
							QBCore.Functions.Notify("You need a Hacking Device to hack this", "error")
						end
					end, "hacking_device")
				else
...
function HackSuccess()
    ...
    ClearPedTasksImmediately(PlayerPedId())
	RobbingTheMoney()
	TriggerServerEvent("atmrobbery:success")	
    TriggerServerEvent('atmrobbery:Server:BeginCooldown')
end
...
local prop = {
  ...
  exports['qb-target']:AddTargetModel(prop, {
      options = {
          {
              type = "client",
              event = "atmrobbery:client:roubar",
